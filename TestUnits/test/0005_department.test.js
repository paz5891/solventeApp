...
describe('Departamento: ', () => {
    ...
    it('Paso 2 OBTENER EL VALOR INGRESADO EN EL PASO 1', (done) => {
        chai.request(url)
            ...
            .end(function (err, res) {
                id = res.body[Object.keys(res.body).length - 1].id_department;
    ...
    it('Paso 3 actualizamos el registro obtenido del paso 2', (done) => {
        chai.request(url)
            .put('/departmentcatalog')
            .send({ id_department: id, department: "PRUEBA 2" })
            .end(function (err, res) {
                chai.request(url)
                    .get('/departmentcatalog')
                    .end(function (err, res) {
                        expect(res.body[Object.keys(res.body).length - 1]).to.have.property('department').to.be.equal('PRUEBA 2');
                        done();
                    });
            });
    })
    it('Paso 4 eliminar un registro de la tabla de departamento', (done) => {
        chai.request(url)
            .delete('/departmentcatalog/' + id)
            .end(function (err, res) {
                chai.request(url)
                    .get('/departmentcatalog')
                    .end(function (err, res) {
